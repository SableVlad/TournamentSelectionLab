...
class Fconst:
    ...
    @staticmethod
    def generate_optimal(length):
        return Chromosome(np.ones((length,), dtype=int), length)
    
    def get_optimal(self, n, l, p_m, i):
        return self.generate_optimal(l)

    def generate_population(self, n, l, p_m):
        return PopulationFactory(self).generate_population(n, l, p_m, 0)
...
class Fe025x:
    @staticmethod
    def estimate(chromosome):
        return math.exp(.25 * 10.23) - math.exp(decode(chromosome, 0, 10.23, len(chromosome)) * .25)

    @staticmethod
    def get_genotype_value(chromosome_code):
        return decode(chromosome_code, 0, 10.23, len(chromosome_code))

    @staticmethod
    def get_optimal(n, l, p_m, i):
        return Fe025x.generate_optimal(l)

    @staticmethod
    def generate_optimal(length):
        x = 0
        gray_code = encode(x, 0, 10.23, length)
        return Chromosome(gray_code, math.exp(.25 * x))

    def generate_population(self, n, l, p_m, i):
        return PopulationFactory(self).generate_population_fe025x(n, l, p_m, i)

    @staticmethod
    def check_chromosome_success(ch: Chromosome):
        x = Fe025x.get_genotype_value(ch.code)
        return ((math.exp(.25 * x) - ch.fitness) <= DELTA) and abs(decode(ch.code, 0, 10.23, len(ch.code))) <= SIGMA

class Fe1x:
    @staticmethod
    def estimate(chromosome):
        return math.exp(10.23) - math.exp(decode(chromosome, 0, 10.23, len(chromosome)))
    ...
    @staticmethod
    def get_optimal(n, l, p_m, i):
        return Fe1x.generate_optimal(l)

    @staticmethod
    def generate_optimal(length):
        x = 0
        gray_code = encode(x, 0, 10.23, length)
        return Chromosome(gray_code, math.exp(x))
    ...
    @staticmethod
    def check_chromosome_success(ch: Chromosome):
        x = Fe1x.get_genotype_value(ch.code)
        return ((math.exp(x) - ch.fitness) <= DELTA) and abs(decode(ch.code, 0, 10.23, len(ch.code))) <= SIGMA
    
class Fe2x:
    @staticmethod
    def estimate(chromosome):
        return math.exp(2 * 10.23) - math.exp(decode(chromosome, 0, 10.23, len(chromosome)) * 2)
    ...
    @staticmethod
    def get_optimal(n, l, p_m, i):
        return Fe2x.generate_optimal(l)

    @staticmethod
    def generate_optimal(length):
        x = 0
        gray_code = encode(x, 0, 10.23, length)
        return Chromosome(gray_code, math.exp(2 * x))
    ...
    @staticmethod
    def check_chromosome_success(ch: Chromosome):
        x = Fe2x.get_genotype_value(ch.code)
        return ((math.exp(2 * x) - ch.fitness) <= DELTA) and abs(decode(ch.code, 0, 10.23, len(ch.code))) <= SIGMA
# %%
