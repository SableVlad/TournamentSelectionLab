...
def main(fitness_function, selection_functions: [], file_name, *args):
    ...
    for i in range(0, MAX_RUNS):
        ...
        for selection_function in selection_functions:
            ...
            if file_name != 'FconstALL':
                optimal = fitness_function.get_optimal(*args, i=i)
            else:
                optimal = fitness_function.generate_optimal(100)
            
            folder_name = file_name if file_name is not None else ff_name
            current_run = EvoAlgorithm(Population(p.chromosomes.copy(), p.p_m), sf, fitness_function, optimal).run(i, folder_name, 5)
            if i in range(0, 5) or i == MAX_RUNS:
