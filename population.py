...
from constants import N, EPS, CROSSOVER
...
class Population:
    ...
    def crossover(self, fitness_function):
        if not CROSSOVER:
            return
        self.update()
        for j in range(0, len(self.chromosomes)-1, 2):
            j_len = len(self.chromosomes[j].code)
            crossover_point = random.randint(1, min(j_len, len(self.chromosomes[j+1].code)))
            for i in range(crossover_point, min(j_len, len(self.chromosomes[j+1].code))):
                temp = self.chromosomes[j].code[i]
                self.chromosomes[j].code[i] = self.chromosomes[j+1].code[i]
                self.chromosomes[j+1].code[i] = temp
            for i in range(0, crossover_point):
                temp = self.chromosomes[j+1].code[i]
                self.chromosomes[j+1].code[i] = self.chromosomes[j].code[i]
                self.chromosomes[j].code[i] = temp
            self.chromosomes[j].fitness = fitness_function.estimate(self.chromosomes[j].code)
            self.chromosomes[j+1].fitness = fitness_function.estimate(self.chromosomes[j+1].code)
        self.update()
